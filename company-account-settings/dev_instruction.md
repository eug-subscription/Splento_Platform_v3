...
// ❌ WRONG: Missing compound structure
<Button>Submit</Button> // Missing Button.Icon if needed
```
...
function CTAButton({ intent, ...props }) {
  ...
  return <Button variant={variantMap[intent]} {...props} />;
}
```
...
> **Remember:** HeroUI v3 is fundamentally different from v2. Always use compound components, semantic variants, and composition patterns. When in doubt, check Rule #0 and consult the reference documents.
